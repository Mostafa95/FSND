...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_create_question(self):
        res = self.client().post('/questions',
                 json={"question":"can you?", "answer":"nope","category":"3","difficulty":"1"})
        body = json.loads(res.data)
        self.assertEqual(res.status_code,200)
        self.assertEqual(body['success'],True)
    ...
    def test_422_wrong_category_create_question(self):
        res = self.client().post('/questions',
                 json={"question":"can you?", "answer":"nope","category":"1000","difficulty":"1"})
        body = json.loads(res.data)
        self.assertEqual(res.status_code,404)
        self.assertEqual(body['success'],False)
    ...
    def test_404_quizzes_get_by_category_wrong_ID(self):
        res = self.client().post('/quizzes',
                    json={"previous_questions":"","quiz_category":{"id":"100","type":"Any"}})
        body = json.loads(res.data)
        self.assertEqual(res.status_code,404)
        self.assertEqual(body['success'],False)
    ...
    def test_quizzes_get_by_category_new_category_ID(self):
        ...
        id = id.format()
        res = self.client().post('/quizzes',
                    json={"previous_questions":"","quiz_category":{"id":str(id['id']),"type":id['type'] }})
        body = json.loads(res.data)
        self.assertEqual(res.status_code,200)
        self.assertEqual(body['success'],True)
    ...
    def test_quizzes_get_by_category_with_previous_question(self):
        res = self.client().post('/quizzes', 
                    json={"previous_questions":[{"id":"5"}],"quiz_category":{"id":"1","type":"Science" }})
        body = json.loads(res.data)
        self.assertEqual(res.status_code,200)
        self.assertEqual(body['success'],True)
